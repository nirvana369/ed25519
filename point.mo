...
module Point = {
    ...
    public class Point(xP: Int, yP: Int) = this {
        ...
        public func getWindowSize() : Nat {
            return Option.unwrap<Nat>(_WINDOW_SIZE);
        };
        ...
        */
        ...
        public func add(other: Point) : Point {
            return ExtendedPoint.fromAffine(this).add(ExtendedPoint.fromAffine(other)).toAffine(null);
        };
        ...
        */
        public func multiply(scalar: Int): Point {
            return ExtendedPoint.fromAffine(this).multiply(scalar, ?this).toAffine(null);
        };
...
module ExtendedPoint = {
    ...
    public func fromAffine(p: Point.Point): ExtendedPoint {
        // if (!(p instanceof Point)) {
        // throw new TypeError('ExtendedPoint#fromAffine: expected Point');
        // }
        if (Point.getZero().equals(p)) return getZero();
    ...
    public class ExtendedPoint(
        ...
        func toAffineBatch(points: [ExtendedPoint]): [Point.Point] {
            let toInv = Utils.invertBatch(Array.map<ExtendedPoint, Int>(points, func p = p.z), CONST.CURVE.P);
            var i = 0;
            let ret = Array.map<ExtendedPoint, Point.Point>(points, func p : Point.Point {
        ...
        public func normalizeZ(points: [ExtendedPoint]): [ExtendedPoint] {
            return Array.map<Point.Point, ExtendedPoint>(toAffineBatch(points), func p = fromAffine(p));
        ...
        private func wNAF(n: Int, affPoint: ?Point.Point): ExtendedPoint {
            ...
            if (precomputes == false) {
                ...
                if (affinePoint and W != 1) {
                    precomputes := normalizeZ(precomputes);
        ...
        public func multiply(scalar: Int, affinePoint: ?Point.Point): ExtendedPoint {
        ...
        public func toAffine(invertZ: ?Int): Point.Point {
            let { x; y; z } = this;
            ...
            if (is0) return Point.getZero();
            if (zz != CONST._1n) Debug.trap("invZ was invalid");
            return Point.Point(ax, ay);
        };
...
module RistrettoPoint = {
    // let BASE = RistrettoPoint(ExtendedPoint.BASE);
    // let ZERO = RistrettoPoint(ExtendedPoint.ZERO);

    public func getZero() : RistrettoPoint {
        return RistrettoPoint(ExtendedPoint.getZero());
    };

    public func getBase() : RistrettoPoint {
        return RistrettoPoint(ExtendedPoint.getBase());
    };

    public class RistrettoPoint(epParams: ExtendedPoint.ExtendedPoint) = this {
        ...
        private func calcElligatorRistrettoMap(r0: Int): ExtendedPoint.ExtendedPoint {
            let { d } = CONST.CURVE;
            ...
            return ExtendedPoint.ExtendedPoint(Utils.mod(W0 * W3, null), Utils.mod(W2 * W1, null), Utils.mod(W1 * W3, null), Utils.mod(W0 * W2, null));
        };
        ...
        */
        private func fromHex(h: T.Hex): RistrettoPoint {
            ...
            return RistrettoPoint(ExtendedPoint.ExtendedPoint(x, y, CONST._1n, t));
        };
